  ...
  return (
    <div className={styles.container}>
      ...
      <main className={clsx(styles.main, sartoshiFont.className)}>
        ...
        {secretSantaIsSuccess && <a href={`${process.env.NEXT_PUBLIC_ETHERSCAN_LINK}${secretSantaData?.hash}`} target="_blank" rel="noopener noreferrer" className={styles.link}>View on Etherscan</a>}
        {data != undefined && data > 0 && <Button disabled={!data} onClick={() => { secretSantaWrite?.() }} className={styles.glowOnHover}>{secretSantaIsLoading ? <CircularProgress /> :  'Surprise Me'}</Button>}
        {<Button 
          onClick={() => { 
            setTriggerNftLoad(true);
            refetchBags();
